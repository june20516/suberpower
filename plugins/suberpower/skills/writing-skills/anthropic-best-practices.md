...
좋은 Skill은 간결하고, 잘 구조화되어 있으며, 실제 사용으로 테스트됩니다. 이 가이드는 Claude가 효과적으로 발견하고 사용할 수 있는 Skill 작성에 도움이 되는 실용적인 작성 지침을 제공합니다.
...
Claude가 아직 갖고 있지 않은 context만 추가합니다. 정보 하나하나를 다음 질문으로 따져 보세요:
...
작업의 취약성과 가변성에 따라 구체성 수준을 조절합니다.
...
Skill은 모델을 보완하는 요소로 작동하므로, 효과는 기본 모델에 달려 있습니다. 사용할 계획인 모든 모델로 Skill을 테스트하세요.
...
Opus에서 완벽히 작동하는 지시도 Haiku에서는 더 많은 디테일이 필요할 수 있습니다. 여러 모델에서 Skill을 사용할 계획이라면, 모두에서 잘 작동하는 지시를 목표로 하세요.
...
<Note>
  ...
  * `name` - 사람이 읽기 쉬운 Skill 이름 (최대 64자)
...
<Warning>
  **항상 3인칭으로 작성하세요**. description은 system prompt에 주입되므로, 인칭이 일관되지 않으면 Skill 발견에 문제가 생길 수 있습니다.
...
각 Skill은 정확히 하나의 description 필드를 가집니다. description은 skill 선택에 결정적입니다: Claude는 100개가 넘을 수도 있는 Skill 중에서 올바른 Skill을 선택하기 위해 이것을 사용합니다. description은 Claude가 언제 이 Skill을 선택할지 알 수 있도록 충분한 디테일을 제공해야 하며, SKILL.md의 나머지가 구현 디테일을 제공합니다.
...
참조된 파일이 또 다른 파일을 참조하면, Claude는 그 파일을 일부만 읽을 수 있습니다. 이런 중첩 참조를 만나면 Claude는 전체 파일을 읽는 대신 `head -100` 같은 명령으로 내용을 미리 훑는 데 그치고, 결국 불완전한 정보를 갖게 됩니다.
...
그러면 Claude는 파일 전체를 읽거나 필요에 따라 특정 섹션으로 이동할 수 있습니다.
...
**예시 1: 리서치 종합 워크플로우** (코드 없는 Skill용):
...
1. 수정 유형을 판단한다:
   ...
   **새 콘텐츠를 만드나요?** → 아래 "생성 워크플로우"를 따른다
   **기존 콘텐츠를 편집하나요?** → 아래 "편집 워크플로우"를 따른다
...
**광범위한 문서를 작성하기 전에 평가를 만드세요.** 이는 Skill이 가상의 문제가 아니라 실제 문제를 해결하도록 보장합니다.
...
<Note>
  이 예시는 단순한 테스팅 루브릭이 있는 데이터 주도 평가를 보여줍니다. 현재 이러한 평가를 실행하는 내장 방법은 제공하지 않습니다. 사용자가 자신의 평가 시스템을 만들 수 있습니다. 평가는 Skill의 효과를 측정하는 기준점입니다.
</Note>

### Claude와 함께 반복적으로 Skill 개발

가장 효과적인 Skill 개발 프로세스에는 Claude 자신이 참여합니다. Claude의 한 인스턴스("Claude A")와 작업하여 다른 인스턴스("Claude B")가 사용할 Skill을 만듭니다. Claude A는 당신이 지시를 설계하고 다듬는 것을 돕는 동안, Claude B는 실제 작업에서 그것을 테스트합니다. 이것이 작동하는 이유는 Claude 모델이 효과적인 agent 지시를 작성하는 방법과 agent가 어떤 정보가 필요한지 모두 이해하기 때문입니다.
...
Skill을 개선할 때도 같은 협업 구조가 그대로 이어집니다. 다음 사이를 번갈아 합니다:
...
2. **Claude B의 행동 관찰**: 어려움을 겪는 곳, 성공하는 곳, 예상치 못한 선택을 하는 곳을 기록합니다
   ...
   **관찰 예시**: "Claude B에게 지역 매출 보고서를 요청했을 때, Skill이 이 규칙을 언급했음에도 테스트 계정을 필터링하는 걸 잊었어."
...
* **무시된 콘텐츠**: Claude가 번들된 파일에 결코 접근하지 않으면, 불필요하거나, 메인 지시에서 충분히 부각되지 않았을 수 있습니다
