...
어떻게 하시겠습니까:
A) ~/.claude/skills/coding/ 에서 리팩터링 지침 확인
B) 그냥 리팩터링 - 어차피 방법을 다 알고 있음
```
...
어떤 작업이든 시작하기 전에 반드시 skill을 확인하세요!

프로세스:
1. 작업을 시작하나요? 확인: `ls ~/.claude/skills/[category]/`
2. skill을 찾았나요? 진행하기 전에 READ IT COMPLETELY
3. skill의 지침을 따르세요 - 알려진 함정을 방지해 줍니다
...
- agent가 압박 하에서도 skill 지침을 따름
- agent가 합리화로 준수를 회피할 수 없음

**변형이 실패하는 경우:**
- 압박이 없어도 agent가 확인을 건너뜀
- agent가 읽지도 않고 "개념만 대충 응용"함
...
**Variant C:** 준수도는 높지만 너무 경직되게 느껴질 수 있음

**Variant D:** 균형 잡혔지만 길다 - agent가 내재화할까?
...
4. 준수율 비교
5. 어떤 합리화가 방어를 뚫는지 식별
6. 가장 효과적인 변형을 반복 개선하여 허점 차단
